//SyncSix_Remote Arduino Sketch. Designed for use with another controller with SyncSix_Master Arduino Sketch. 
//Allows many controllers to be triggered simultaneously for controlling more than 6 outputs.
//The Master controller has all the same functionality as the SyncSix_V1 Arduino Sketch with the addition that the D3 pin sends out a 5v trigger pulse when triggered     
//Remote controllers can receive this trigger pulse from the Master controller via the ultrasonic TRIG screw terminal
//Remote controllers will also send out a trigger pulse on the ultrasonic ECHO screw terminal for triggering more controllers
//Remote controllers can all be wired to the masters D3 pin or daisy-chained off one another, depending on your wiring preference
//If you are using several different power supplies you may need to also connect the grounds of each controller together to prevent false triggering. 
...
void setup() {
  ...
  pinMode(utrig, INPUT);    // Set the ultrasonic trigPin as an INPUT to recieve trigger signal from Master controller
  pinMode(uecho, OUTPUT);   digitalWrite(uecho, LOW);       // Set the ultrasonic echoPin as an OUTPUT to trigger another remote
...
void loop() {
  ...
  if ((trigstate||sonicstate) & !shiftstate) {            //if triggered, play sequence
    digitalWrite(uecho, HIGH);  delay(1); //trigger other remote controllers conected to the echo pin
    digitalWrite(led, HIGH);  //turn on led for duration of playback
    digitalWrite(uecho, LOW);   //reset echo pin
...
const int inputThres = 800;  //analog read threshold value utrig pin must reach in order to trigger controller (value of 300 is about 1.5V)
...
void inputCheck() {         //refreshes input button states and trig
  ...
  if (analogRead(utrig) > inputThres){        //if trigger from master controller is recieved
    trigstate = HIGH;                       //set trigstate hight to trigger controller
  }
}
