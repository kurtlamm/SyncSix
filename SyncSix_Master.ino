//SyncSix_Master Arduino Sketch. Designed for use with other controllers with SyncSix_Remote Arduino Sketch. 
...
void setup() {
  pinMode(remoteTrig, OUTPUT);  digitalWrite(remoteTrig, LOW);      //setup remote trigger pin D3 and set low
...
void loop() {
  ...
  if ((trigstate||sonicstate) & !shiftstate) {            //if triggered, play sequence
    digitalWrite(remoteTrig, HIGH);  delay(1); //trigger remote controllers conected to the D3 pin    
    digitalWrite(led, HIGH);  //turn on led for duration of playback
    digitalWrite(remoteTrig, LOW);   //reset D3 remote trigger pin
