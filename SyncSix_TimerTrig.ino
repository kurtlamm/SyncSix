//SyncSix_TimerTrig Arduino Sketch. 
//Allows a SyncSix controller to be periodically triggered every X (timerPeriod) amount of seconds. 
//ResetTimerOnTrig can be set HIGH to reset the timer if the controller is externally triggered. 

const unsigned long timerPeriod = 60;           //set how often the controller should trigger the sequence (in seconds)
const int resetTimerOnTrig = HIGH;              //Set HIGH if you want the timer to be reset if the contller is externally triggered  
...
unsigned long timerPeriodms = timerPeriod * 1000;
unsigned long lastTimerTrig;
...
void setup() {
  ...
  lastTimerTrig = millis();   //record Timer Trig
...
void loop() {
  ...
  if (millis() > (lastTimerTrig+timerPeriodms)) {
      lastTimerTrig = millis();   //record Timer Trig
      digitalWrite(led, HIGH);  //turn on led for duration of playback
      playSeq();                //play sequence
  }
  ...
  if ((trigstate||sonicstate) & !shiftstate) {            //if triggered, play sequence
    if (resetTimerOnTrig) {lastTimerTrig = millis();}   //if reset timer on trigger, reset the timer
    digitalWrite(led, HIGH);  //turn on led for duration of playback
    playSeq();                //play sequence
  }
