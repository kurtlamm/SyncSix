...
const unsigned int numOfSeq = 6;      //define number of sequences the controller will record/playback
...
unsigned int memBlockSize = 65534/numOfSeq;     //define memory block allocated per sequence
unsigned int currMem = 0;                       //global var to store start address of sequence memory block
unsigned int currSeq = 1;                       //global var to store current sequence 
...
void loop() {
  ...
  if ((trigstate||sonicstate) & !shiftstate) {            //if triggered, play sequence
    ...
    currSeq++;
    if (currSeq == (numOfSeq+1)) {currSeq=1;}
    currMem = (memBlockSize*currSeq)-(memBlockSize);    
  }
  ...
  if (EEPROM.read(ambientbyte) && (analogRead(busy)>500)) {    //if play ambient track and ambient track has stoped
    playMP3(numOfSeq+1);     //play last MP3 (ambient)
    delay(25);
  }
...
void playSeq(){
  unsigned long currentmemory = currMem;
  playMP3(currSeq);       //play Xth mp3 file
  ...
  while (keepGoin) {
    ...
    while ((millis()-startmillis) < ((currentmemory-currMem+1)*quant)){}        //wait to execute
...
void recSeq(){
  ...
  while((!shiftstate)  && (currentmemory<(currMem+memBlockSize-1))) {          //loop while user hasnt pressed shift button and there is still memory space left
    while((millis()-startmillis) < ((currentmemory-currMem+1)*quant)) {}       //wait to execute
...
void recSeqLayer(){
  unsigned long currentmemoryL = currMem;
  byte instructL = 0;      //
  playMP3(currSeq);          //play Xth mp3 file
  ...
  while(!(bitRead(instructL, 7) == 1)) {          //loop untill end of sequence
    while((millis()-startmillisL) < ((currentmemoryL-currMem+1)*quant)) {}
...
void playMP3(unsigned int track) {        //plays MP3 tracks
  byte playMP3[] = {0x7E, 0xFF, 0x06, 0x03, 0x00, 0x00, track, 0xEF};           //message to send to MP3 player to play track 1
  DFSoftwareSerial.write(playMP3, 8);      //send code to play MP3
}
