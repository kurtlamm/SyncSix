//SyncSix Ultrasonic CH6 Sketch. This sketch uses the ultrasonic sensor to turn off channel 6 when below the threshold set by the potentiometer.
//Sequences can only be triggered by standard tirgger input.
//The variable "sonicmillisR" is the amount of time (in ms) ch6 will be off since the last ultrasonic trigger. This helps ch6 from flickering when turning ch6 off.

unsigned long sonicmillisR = 1000;     //time (in ms) that ch6 will wait to reset after being triggered
...
unsigned long sonicmillis = millis();     //record start time

void loop() {
  inputCheck();   //refresh the inputs

  
  if ((trigstate) & !shiftstate) {            //if triggered, play sequence
  ...
  if (sonicstate ){
    sonicmillis = millis();     //record time ultrasonic sensor was last triggered
    digitalWrite(ch6, LOW);
  }
  else if ((millis()-sonicmillis)>sonicmillisR){digitalWrite(ch6, HIGH);}
