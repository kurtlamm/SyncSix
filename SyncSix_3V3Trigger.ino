//SyncSix_3V3Trigger Arduino Sketch. 
...
int analogthreshV = map(thresholdV, 0.0, 5.0, 0, 1023);
...
void setup() {
  ...
  pinMode(utrig, INPUT);   // Set the ultrasonic trigPin as an input
...
void inputCheck() {         //refreshes input button states and trig
  ...
  if (analogRead(utrig) > analogthreshV){        //if voltage on uTrig pin is higher then the threshold
     trigstate = HIGH;                      //trigger the controller
  }
}
