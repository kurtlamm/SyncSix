//SyncSix Ambient Amination Arduino Sketch  
//Allows channels to be simply animated while the controller is untriggered (ambient mode) 
//Variable AAtime sets the update interval time (in ms) between steps for the ambient animation
//Variable AAch defines the ambient animation pattern. You can add as many steps as desired
//channel order is as follows: BXXXXXX = Bch6ch5ch4ch3ch2ch1. Set bit to 1 to turn channel on, 0 for off.

const unsigned int AAtime = 1000;
const byte AAch[] = {B100000, B010000}; 
...
unsigned long AAlasttime = millis(); 
unsigned int AAstep = 0;

void loop() {
  inputCheck();   //refresh the inputs


  if ( (millis()-AAlasttime) > AAtime) {
    updateCh(AAch[AAstep]);
    AAstep++;
    if (AAstep == sizeof(AAch)) {AAstep = 0;}
    AAlasttime = millis(); 
  }
